#ifndef INCLUDE_OUTETTS_H_
#define INCLUDE_OUTETTS_H_
...
namespace edge {
    ...
    class Outetts {
        public:
            Outetts(const std::string& ttc_model_path, const std::string& cts_model_path);
            
            // 新增：处理文本到语音的全流程
            bool text_to_speech(const std::string& text, std::vector<float>& audio_out);

            // 新增：保存WAV文件
            bool save_wav(const std::string& fname, const std::vector<float>& data);
            
            ~Outetts(){
                // if (model_ttc_ != nullptr) {
                //     llama_model_free(model_ttc_);
                //     model_ttc_ = nullptr;
                // }
                // if (ctx_ttc_ != nullptr) {
                //     llama_free(ctx_ttc_);
                // }
                // if (model_cts_ != nullptr) {
                //     llama_model_free(model_cts_);
                //     model_cts_ = nullptr;
                // }
                // if (ctx_cts_ != nullptr) {
                //     llama_free(ctx_cts_);
        ...
        private:
            ...
            void init_params_ttc(const std::string& ttc_model_path);
            void init_params_cts(const std::string& cts_model_path);
            void init_params(const std::string& ttc_model_path, const std::string& cts_model_path);
            // 模型参数
            common_params params_ttc_;
            // const llama_vocab * vocab_ttc_ = nullptr;

            common_params params_cts_;
            

            common_init_result llama_init_ttc;
            common_init_result llama_init_cts;
            // 模型和上下文
            llama_model* model_ttc_ = nullptr;  // 文本到编码模型
            llama_context* ctx_ttc_ = nullptr;

            llama_model* model_cts_ = nullptr;  // 编码到语音模型
            llama_context* ctx_cts_ = nullptr;
            
            // 采样器
            common_sampler* sampler_ = nullptr;
            
            // TTS版本
            outetts_version tts_version_ = OUTETTS_V0_2;
            
            // 音频合成参数
            int n_parallel_ = 1;
            int n_predict_ = 4096;
            int save_sample_rate_ = 24000;
            int n_fft_ = 1280;
            int n_hop_ = 320;

            std::vector<llama_token> codes_;
            std::vector<llama_token> guide_tokens_;
            std::string audio_text_ = "<|text_start|>the<|text_sep|>overall<|text_sep|>package<|text_sep|>from<|text_sep|>just<|text_sep|>two<|text_sep|>people<|text_sep|>is<|text_sep|>pretty<|text_sep|>remarkable<|text_sep|>sure<|text_sep|>i<|text_sep|>have<|text_sep|>some<|text_sep|>critiques<|text_sep|>about<|text_sep|>some<|text_sep|>of<|text_sep|>the<|text_sep|>gameplay<|text_sep|>aspects<|text_sep|>but<|text_sep|>its<|text_sep|>still<|text_sep|>really<|text_sep|>enjoyable<|text_sep|>and<|text_sep|>it<|text_sep|>looks<|text_sep|>lovely<|text_sep|>";
            std::string audio_data_ = R"(<|audio_start|>
